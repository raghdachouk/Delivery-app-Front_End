...
import { View, StyleSheet, Text, TextInput, KeyboardAvoidingView, Keyboard } from 'react-native';
...
export default class Register extends React.PureComponent {
	...
	componentDidUpdate(_, { success }) {
		if (!success && this.state.success) {
			setTimeout(() => {
				this.props.navigation.dispatch(NavigationActions.navigate({ routeName: 'FirstStep' }));
			}, 1000);
		}
	}
	...
	handleSubmit = () => {
		...
		if (!loading && NAME_REGEX.test(firstName) && NAME_REGEX.test(lastName)) {
			const { email, password, mobile, countryCode } = this.props.navigation.state.params;
			...
			register({
				...
				.then(({ success, message }) => {
					...
					// if(success) this.props.navigation.navigate('Home')
				})
				.catch((e) => {
					Keyboard.dismiss();
					this.setState({
				...
				<View style={style.container}>
					...
					<View style={styles.footer}>
						<Button
							...
							loading={loading}
							disabled={!NAME_REGEX.test(firstName) || !NAME_REGEX.test(lastName)}
						/>
