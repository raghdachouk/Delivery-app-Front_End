...
export default class Password extends React.PureComponent {
	...
	handleSubmit = () => {
		...
		if (password.trim().length >= 5) {
			const { email, mobile, countryCode, isUser } = this.props.navigation.state.params;
			if (isUser) {
				...
				<View style={style.container}>
					<KeyboardAvoidingView style={style.subContainer}>
						<Text style={style.label}>
							{!isUser ? 'Créez le mot de passe de votre compte' : 'Connectez-vous pour continuer'}
						</Text>
					...
					<View style={styles.footer}>
						<Button
							...
							loading={loading}
							disabled={password.trim().length < 5}
						/>
					</View>
